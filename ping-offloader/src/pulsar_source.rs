...
use serde::Serialize;
use ping_data::pulsar_messages::{CheckData, CheckMessage, CheckType};
...
impl<A: Serialize + DeserializeOwned + Debug> PulsarSource<A> {
    pub async fn new(connection_data: &PulsarConnectionData, sender: Sender<CheckData<A>>, check_type: CheckType) -> Option<Self> {
        ...
        info!("Starting consumer with subscription id : {}", &subscription_name);
    ...
    pub async fn run(&mut self) {
        while let Some(message) =
            ...
            let _ = match self.sender.send(check_data) {
                Ok(_) =>
                    self.consumer.ack(&message).await,
                Err(error) => {
                    error!("Could not send data to channel : {:?}", error);
                    continue;
                }
            };
        }
