use clap::Parser;
use isok_offloader::config::Config;
use isok_offloader::run;
...
struct CliArgs {
    #[arg(short, long = "config", env = "ISOK_OFFLOADER_CONFIG_PATH")]
...
impl CliArgs {
    fn get_possible_paths(bin_name: &str) -> Vec<PathBuf> {
        vec![
            PathBuf::from(format!("/etc/{bin_name}/offloader.toml")),
            PathBuf::from(format!("./{bin_name}/offloader.toml")),
            PathBuf::from("./offloader.toml"),
        ]
    }
}
