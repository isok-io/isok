...
struct CliArgs {
    #[arg(short, long = "config", env = "ISOK_BROKER_CONFIG_PATH")]
...
impl CliArgs {
    fn get_possible_paths(bin_name: &str) -> Vec<PathBuf> {
        vec![
            PathBuf::from(format!("/etc/{bin_name}/broker.toml")),
            PathBuf::from(format!("./{bin_name}/broker.toml")),
            PathBuf::from("./broker.toml"),
        ]
    }
}
